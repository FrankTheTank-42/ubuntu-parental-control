...
(() => {
  ...
  const sourcePriority = document.querySelector("#block-source-priority");
  ...
  api.runtime.sendMessage({ type: "get_block_info", block_id: blockId }).then((response) => {
    ...
    sourcePriority.textContent = Number.isInteger(response.result.priority)
      ? `Priorität ${response.result.priority}`
      : "Standardaktion";
    source.hidden = false;
  }).catch(() => {
